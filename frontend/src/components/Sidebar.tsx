...
export default function Sidebar({ anomalies, stats, realPositions }: SidebarProps) {
  return (
    <div className="w-[450px] shrink-0 border-r border-white/10 bg-slate-950/80 backdrop-blur flex flex-col z-10 p-5 gap-6 overflow-y-auto custom-scrollbar">
      ...
      {/* Generated Metrics Container via Backend Matplotlib */}
      <div className="space-y-6 mt-4 pb-6">
        ...
        <div className="space-y-4">
          ...
          <div className="p-2 rounded-xl border border-white/10 bg-slate-900 shadow-lg relative aspect-[4/3] overflow-hidden group">
           ...
           <div className="p-2 rounded-xl border border-white/10 bg-slate-900 shadow-lg relative aspect-[4/3] overflow-hidden group">
             <div className="absolute top-2 left-2 z-10 text-[10px] uppercase font-bold text-white/50 bg-black/50 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity">Fig 4. ATSAD Bench Evaluation</div>
             <img src={getGraphUrl('atsad-metrics')} alt="ATSAD Bench Metrics" className="w-full h-full object-contain mix-blend-screen" />
          </div>
          
          <div className="p-2 rounded-xl border border-white/10 bg-slate-900 shadow-lg relative aspect-[10/6] overflow-hidden group">
             <div className="absolute top-2 left-2 z-10 text-[10px] uppercase font-bold text-white/50 bg-black/50 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity">Fig 5. Space Weather Correlation</div>
             <img src={getGraphUrl('space-weather')} alt="Space Weather Correlation" className="w-full h-full object-contain mix-blend-screen" />
          </div>
        </div>
      </div>
