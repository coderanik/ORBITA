...
async def get_conjunction(conjunction_id: int, db: AsyncSession = Depends(get_db)):
    """Get a single conjunction event by ID."""
    result = await db.execute(
        select(ConjunctionEvent).where(ConjunctionEvent.conjunction_id == conjunction_id)
    )
...
async def create_conjunction(
    ...
    event = ConjunctionEvent(**payload.model_dump(exclude_unset=True))
    db.add(event)
    await db.flush()
