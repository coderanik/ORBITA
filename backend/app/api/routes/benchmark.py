...
async def list_datasets(
    ...
    query = enforce_org_scope(select(BenchmarkDataset), BenchmarkDataset, current_user)
    count_query = enforce_org_scope(select(func.count()).select_from(BenchmarkDataset), BenchmarkDataset, current_user)
...
async def get_dataset(
    ...
    query = enforce_org_scope(select(BenchmarkDataset), BenchmarkDataset, current_user).where(
        BenchmarkDataset.dataset_id == dataset_id
    )
...
async def list_models(
    ...
    query = enforce_org_scope(select(BenchmarkModel), BenchmarkModel, current_user)
    count_query = enforce_org_scope(select(func.count()).select_from(BenchmarkModel), BenchmarkModel, current_user)
...
async def get_model(
    ...
    query = enforce_org_scope(select(BenchmarkModel), BenchmarkModel, current_user).where(
...
async def list_runs(
    ...
    query = enforce_org_scope(select(BenchmarkRun), BenchmarkRun, current_user)
    count_query = enforce_org_scope(select(func.count()).select_from(BenchmarkRun), BenchmarkRun, current_user)
...
async def get_run(
    ...
    query = enforce_org_scope(select(BenchmarkRun), BenchmarkRun, current_user).where(BenchmarkRun.run_id == run_id)
    result = await db.execute(query)
    obj = result.scalar_one_or_none()
    if not obj:
...
async def get_run_results(
    ...
    run = (
        await db.execute(
            enforce_org_scope(select(BenchmarkRun), BenchmarkRun, current_user).where(BenchmarkRun.run_id == run_id)
        )
    ).scalar_one_or_none()
    if not run:
        raise HTTPException(status_code=404, detail="Run not found")
...
async def create_result(
    ...
    run = (
        await db.execute(
            enforce_org_scope(select(BenchmarkRun), BenchmarkRun, current_user).where(BenchmarkRun.run_id == payload.run_id)
        )
    ).scalar_one_or_none()
    if not run:
        raise HTTPException(status_code=404, detail="Run not found")
...
async def get_leaderboard(
    ...
    if current_user.get("org_id") is not None:
        query = query.where(BenchmarkRun.org_id == current_user.get("org_id"))
